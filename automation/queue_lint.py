...
def lint(path, bad_keys=None):
    ...
    for i, p in enumerate(posts):
        ...
        if yt:
            ...
            # 【2026-08-26 konan「厳しすぎるし極端なゲート辞めろ」「俺が判定器」】
            # 実測傾向ベースの足切り(問いかけ形324/それ以外166・「アプリ」入り48/無し241)は
            # 却下条件から外し、警告表示のみ(作る段階の参考値)。却下してよいのは
            # 嘘・法的リスク・技術的破損だけ([[feedback_rule_bar_legal_or_sales]])。
            if "アプリ" in yt:
                print(f"⚠️  {tag}: yt_titleに「アプリ」(実測48再生 vs 無し241)— 参考警告・却下はしない")
